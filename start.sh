...
BAD_COUNT=$(find /pocketmine/Server/data /pocketmine/Server/plugins ! -user pocketmine | wc -l)
if [ $BAD_COUNT -gt 0 ]; then
	echo "=== WARNING ==="
	echo "Detected $BAD_COUNT files in /data or /plugins not owned by the user \"pocketmine\"!"
	echo "For example:"
	find /pocketmine/Server/data /pocketmine/Server/plugins ! -user pocketmine | head
...
exec /pocketmine/Server/bin/php7/bin/php PocketMine-MP.phar --no-wizard --enable-ansi --data=/pocketmine/Server/data --plugins=/pocketmine/Server/plugins
